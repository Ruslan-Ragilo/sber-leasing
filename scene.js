...
const loadingManager = new THREE.LoadingManager(
  ...
  (itemUrl, itemsLoaded, itemsTotal) => {
    ...
    loadingProgress.style.background = `conic-gradient(from 1rad, #00000000 ${Math.floor(
      progressRatio * 360,
    )}deg, white 0deg)`;
  },
);
...
const imagesWithTexturesInfo = ImagesInfo.map((info) => {
  return {
    ...info,
    urlMap: info.name + ".jpg",
    urlAlpha: info.name + "_a.jpg",
    map: null,
    alphaMap: null,
  };
});

const shadowCarAlpha = textureLoader.load(
  "./scene/textures/images/shadow_car_alpha.jpg",
  (loadedTexture) => {
    loadedTexture.colorSpace = THREE.SRGBColorSpace;
  },
);
imagesWithTexturesInfo.forEach((info) => {
  info.map = textureLoader.load(
    "./scene/textures/images/" + info.urlMap,
    (loadedTexture) => {
      loadedTexture.colorSpace = THREE.SRGBColorSpace;
    },
  );
  info.alphaMap = textureLoader.load(
    "./scene/textures/images/" + info.urlAlpha,
    (loadedTexture) => {
      loadedTexture.colorSpace = THREE.SRGBColorSpace;
    },
  );
...
gltfLoader.load("./scene/models/road.glb", (gltf) => {
  gltf.scene.position.x -= 0.11;
  scene.add(gltf.scene);
});

gltfLoader.load("./scene/models/car.glb", (gltf) => {
  ...
  objectsList.forEach((img) => {
    if (
      img.fadeInStartPosition <= carMoving.position.z ||
      !img.fadeInStartPosition
    ) {
...
imagesWithTexturesInfo.forEach((info) => {
  generateImage(
    { map: info.map, alphaMap: info.alphaMap },
    info.position,
    info.scale,
    info.fadeInStartPosition,
  );
});
...
generateRail([0.12, 0.0, -12.025]);
generateRail([0.14, 0.0, -12.01]);
...
window.addEventListener("scroll", function () {
  scrollPosition = -(
    ...
    (scrollContainer.clientHeight + sizes.height * 2)
  );
...
const fadeInObjects = () => {
  objectsList.forEach((obj) => {
    if (!triggeredObjects.includes(obj)) {
      const distanceZ = calculateDistanceZ(
        carMoving.position.z,
        obj.fadeInStartPosition,
      );
      if (distanceZ < 1) {
...
const BlocksWithTextMovementAndFadeIn = () => {
  for (const block of BlockWithTextInfo) {
    ...
    block.element.style.top =
      translateY * camera.zoom + sizes.height * 0.5 * -(camera.zoom - 1) + "px";
...
const animate = () => {
  ...
  if (carMoving !== null) {
    ...
    const clampedZPosition = Math.min(
      Math.max(carZPosition, cameraMaxFarLookAtZPosition + 7),
      cameraMaxNearLookAtZPosition + 7,
    );
    const clampedLookAtZPosition = Math.min(
      Math.max(carMoving.position.z, cameraMaxFarLookAtZPosition),
      cameraMaxNearLookAtZPosition,
    );
    ...
    mixer.setTime(
      THREE.MathUtils.damp(
        mixer.time,
        carAnimation.duration * scrollPosition,
        lambda,
        deltaTime,
      ),
    );
